...
# Load MobileNetV2 model for feature extraction (optional)
base_model = None
...
def preprocess_image(img: Image.Image) -> np.ndarray:
    ...
    # tf.keras.applications.mobilenet_v2.preprocess_input expects float -1..1 or 0..255 depending on version
...
def extract_features(img: Image.Image) -> np.ndarray:
    ...
    if base_model is None:
        # Fallback: use simple image hash
...
def detect_screenshot(img: Image.Image) -> bool:
    ...
    # Screenshots often have very low noise/variance in flat areas
    # calculating the standard deviation of the image
    stat = ImageStat.Stat(img)
    # If standard deviation is extremely low, might be a solid color or screenshot
    # Real photos usually have higher variance
...
def detect_reprint(img: Image.Image) -> bool:
    ...
    # Calculate variance of edges (pseudo-Laplacian variance)
    stat = ImageStat.Stat(edges)
    variance = sum(stat.var) / len(stat.var)
    
    # Low variance in edges implies blurriness (reprint/scan)
    return variance < 100
...
async def register_device(request: DeviceRegistrationRequest):
    ...
    try:
        ...
        # Simple device identification (in production, use a trained classifier)
...
async def verify_device(request: DeviceVerificationRequest):
    ...
    try:
        ...
        # Extract features from live image (live_img is PIL Image)
        live_features = extract_features(live_img)
        
        # Get registered features from request
        if request.registered_features:
            registered_features = np.array(request.registered_features)
        else:
            # Fallback for legacy data (SHOULD BE AVOIDED IN PROD)
        ...
        if len(registered_features) != len(live_features):
            # Resize or handle mismatch (simple truncation/padding for resilience)
        ...
        # Anomaly scoring
        anomaly_score = 0.0
        # Only add fraud penalty if image match is also low (fraud + low match = suspicious)
        if fraud_detected and image_match_score < 0.8:
            anomaly_score += 0.4
        elif fraud_detected:
            # If fraud detected but high image match, reduce penalty (might be false positive)
            anomaly_score += 0.2
        ...
        # Determine status - be more lenient with high image match scores
        # Block only if: (fraud + low match) OR (very high anomaly + low match)
        ...
        return {
            ...
            "qr_validity": bool(qr_validity),  # Convert to Python bool
            "fraud_detected": bool(fraud_detected),  # Convert to Python bool
